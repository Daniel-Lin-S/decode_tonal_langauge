...
def run(config: dict, config_path: str | None = None) -> None:
    ...
    combined_cfg = {**flat_train, **dataset_cfg, **model_cfg}

    params = dict_to_namespace(combined_cfg)
    ...
    if getattr(params, "separate_models", False):
        results, conf_mat, labels = train_separate_targets(params, seeds, eval_cfg)
    else:
        results, conf_mat, labels = train_joint_targets(params, seeds, eval_cfg)
